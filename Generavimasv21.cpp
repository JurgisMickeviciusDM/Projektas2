...
bool compareNamesWithNumbers(const string& nameA, const string& nameB) {
...
void saveToFile(const list<Studentas>& studentasList, const string& filename, const string& choice1) {
    ...
    if (choice1 == "vardus") {
        sortedStudents.sort([](const Studentas& a, const Studentas& b) {
            return compareNamesWithNumbers(a.getVardas(), b.getVardas());
            });
    }
    else if (choice1 == "pavardes") {
        sortedStudents.sort([](const Studentas& a, const Studentas& b) {
            return a.getPavarde() < b.getPavarde(); // Changed to use getter
            });
    }
